...
class bands(object):
 ...
 def __init__(self, grepen, dos):
  ...
  self.bands=np.float64(listOfBandOfKptE)
  # select bands that cross dos.cbm1+0.5 and dos.vbm-0.5
  self.neargap_bands=[]
  ...
  print 'bandstructure.py: the number of near-gap bands are', len(self.neargap_bands)
  ...
  # -------------------precision check------------------------- 
  ## geometry of k-mesh
  kpoints=[KptE[0:3] for KptE in self.neargap_bands[0]]
  min_kpoint_dist=1
  ...
  def parallelDeltaE(i_band_local):
   ...
   counter_01=0
   counter_03=0
   band=self.neargap_bands[i_band_local]
   for i_kpoint_1 in range(0,len(kpoints)):
  ...
  parallelDeltaE_results = Parallel(n_jobs=20)(delayed(parallelDeltaE)(i_band) for i_band in range(0,len(self.neargap_bands)))
