...
doscar_lines=[tmplines[i].split() for i in range(6,6+NEDOS)]
DOS=np.float64(doscar_lines)
index_fermi=abs(DOS[:,0]-EFERMI).argmin()+1
#ispin=1 or 2?
if LSORBIT!='F':
...
if ISPIN==1: 
 ...
 if abs(DOS[index_fermi][1])>min_dos and abs(DOS[index_fermi][2])>min_dos:
  ...
  if abs(VBM1-VBM2)<min_dos or abs(CBM1-CBM2)<min_dos:
   print "dos.py: insulator. bandgap: %.5f. e-fermi: %.5f. VBM: %.5f. CBM: %.5f." % (CBM1-VBM1, EFERMI, VBM1, CBM1)
  else:
